...
if (isset($_POST['publish'])) {
    // Kiểm tra tất cả các tham số POST bắt buộc
    if (
        isset($_POST['ten_hang'], $_POST['so_luong'], $_POST['ngay_nhap'], $_POST['gia_nhap'], $_POST['gia_ban'], $_POST['nhan_vien_nhap'])
    ) {
        $ten_hang = trim($_POST['ten_hang']);
        $so_luong = trim($_POST['so_luong']);
        $ngay_nhap = trim($_POST['ngay_nhap']);
        ...
        if (
            ...
            try {
                // Thêm một bản ghi mới vào bảng chitiet_nhap_hang mà không cộng dồn số lượng
                $sql_insert = "INSERT INTO chitiet_nhap_hang 
                              (ten_hang, so_luong, ngay_nhap, gia_nhap, gia_ban, nhan_vien_nhap) 
                              VALUES (:ten_hang, :so_luong, :ngay_nhap, :gia_nhap, :gia_ban, :nhan_vien_nhap)";
                $stmt_insert = $conn->prepare($sql_insert);
                $stmt_insert->bindParam(':ten_hang', $ten_hang);
...
<body>
    ...
    <!-- <div class="banner">
        <div class="detail">
            <h1>Quản Lý Kho</h1>
            <span><a href="dashboard.php">Quản Trị</a><i class="bx bx-right-arrow-alt"></i>Nhập Hàng</span>
        </div>
    </div> -->
        ...
        <div class="form-container-warehouse">
            <form action="" method="post" enctype="multipart/form-data" class="register">
                ...
                <div class="flex-btn">
                    ...
                    <a href="chi_tiet_nhap_hang.php" class="btn">Chi tiết Danh Sách Hàng Đã Nhập</a>
                </div>
            </form>
            <!-- <a href="manage_products.php" class="btn">Xem Danh Sách Hàng Đã Nhập</a> -->
        </div>
